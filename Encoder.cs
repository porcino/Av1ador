    ...
    internal class Encoder : ICloneable
    {
        ...
        public void Set_video_codec(string codec)
        {
            ...
            else if (codec == v[5])
            {
                ...
                speed_str = "-preset ";
                Params = "-x265-params min-keyint=!minkey!:keyint=!maxkey!:ssim-rd=1:early-skip=1:psy-rd=0.5:rdoq-level=2:psy-rdoq=2:selective-sao=2:rskip-edge-threshold=1:aq-mode=4:rect=0:subme=2:limit-tu=1:amp=0:hme=1:hme-search=star,umh,hex:hme-range=16,32,100:strong-intra-smoothing=0";
            ...
            if (f == "tonemap")
            {
                ...
                for (int i = 0; i < Vf.Count; i++)
                {
                    if (Vf[i].Contains("tonemap"))
                        d = true;
                    if (Bits == 8 && Vf[i].Contains("format=p010,hwdownload,format=p010"))
                    {
                        Vf[i] = Vf[i].Replace("format=p010,hwdownload,format=p010", "format=nv12,hwdownload,format=nv12");
                        break;
                    }
                    else if (Bits > 8 && Vf[i].Contains("format=nv12,hwdownload,format=nv12"))
                    {
